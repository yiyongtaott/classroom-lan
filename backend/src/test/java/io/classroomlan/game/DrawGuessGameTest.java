...
class DrawGuessGameTest {
    ...
    void testCorrectGuessAddsScore() {
        ...
        // 这里直接触发猜测
        DrawGuessGame.GameActionResult result = game.handleAction("guesser1", "GUESS", Map.of("word", "苹果"));

        assertEquals(DrawGuessGame.GameActionResult.Type.BROADCAST, result.type);
    ...
    void testWrongGuessDoesNotAwardPoints() {
        ...
        DrawGuessGame.GameActionResult result = game.handleAction("guesser1", "GUESS", Map.of("word", "香蕉"));

        assertEquals(DrawGuessGame.GameActionResult.Type.PRIVATE, result.type);
    ...
    void testOnlyDrawerCanSendHints() {
        ...
        DrawGuessGame.GameActionResult result = game.handleAction("other1", "HINT", Map.of("text", "这是提示"));
        assertEquals(DrawGuessGame.GameActionResult.Type.ERROR, result.type);
    ...
    void testDrawerCanSendHint() {
        ...
        DrawGuessGame.GameActionResult result = game.handleAction("drawer1", "HINT", Map.of("text", "提示"));
        assertEquals(DrawGuessGame.GameActionResult.Type.BROADCAST, result.type);
